...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        # Define x-axis values (0 or 1)
        x_values = [0, 1]

        # Create a bar chart
        plt.bar(x_values, counts, color=['blue', 'green'])

        # Label the chart
        plt.title('Results of Coin Flips')  # Replace with an appropriate title
        plt.xlabel('Outcome')  # Replace with an appropriate x-axis label
        plt.ylabel('Count')  # Replace with an appropriate y-axis label
...
#         from math import comb
    ...
    def plot_bar_pdf(self):
        ...
        x_values = list(range(self.n))  # Values of k from 0 to n
        y_values = [self.pdf(self.n, self.p, k) for k in x_values]  # Calculate probabilities for each k
